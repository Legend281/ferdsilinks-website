...
export default async function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Leads', value: stats.totalLeads, subtext: `${stats.leadsThisWeek} this week`, icon: 'inbox', color: 'bg-[#002147]' },
    { label: 'Subscribers', value: stats.totalSubscribers, subtext: 'Newsletter members', icon: 'mail', color: 'bg-[#006a6a]' },
    { label: 'Enrollments', value: stats.totalEnrollments, subtext: `${stats.recentEnrollments} this week`, icon: 'school', color: 'bg-[#cf7000]' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-headline font-bold text-[#002147]">Dashboard</h1>
          <p className="text-gray-500">Welcome back! Here&apos;s an overview of your site.</p>
        </div>
        <Link
          href="/admin/leads"
          className="px-4 py-2 bg-[#cf7000] text-white rounded-lg font-medium hover:bg-[#b86300] transition-all"
        >
          View All Leads
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card) => (
          <div key={card.label} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <h3 className="text-3xl font-headline font-bold text-[#002147]">{card.value}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-headline font-bold text-[#002147]">Recent Leads</h2>
            <Link href="/admin/leads" className="text-sm text-[#cf7000] hover:underline">View all</Link>
          </div>
          <div className="divide-y divide-gray-100">
            {recentLeads.length === 0 ? (
              <div className="px-6 py-12 text-center">
                <span className="material-symbols-outlined text-gray-300 text-4xl">inbox</span>
                <p className="text-gray-500 mt-2">No leads yet</p>
              </div>
            ) : (
              recentLeads.map((lead) => (
                <div key={lead.id} className="px-6 py-4 hover:bg-gray-50 transition-all">
                  <div className="flex items-center justify-between">
                    <div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-headline font-bold text-[#002147]">Recent Enrollments</h2>
            <Link href="/admin/enrollments" className="text-sm text-[#cf7000] hover:underline">View all</Link>
          </div>
          <div className="divide-y divide-gray-100">
            {recentEnrollments.length === 0 ? (
              <div className="px-6 py-12 text-center">
                <span className="material-symbols-outlined text-gray-300 text-4xl">school</span>
                <p className="text-gray-500 mt-2">No enrollments yet</p>
              </div>
            ) : (
              recentEnrollments.map((enrollment) => (
                <div key={enrollment.id} className="px-6 py-4 hover:bg-gray-50 transition-all">
                  <div className="flex items-center justify-between">
                    <div>
                  ...
                  <p className="text-sm text-gray-600 mt-2">{enrollment.course_title}</p>
                </div>
              ))
            )}
          </div>
